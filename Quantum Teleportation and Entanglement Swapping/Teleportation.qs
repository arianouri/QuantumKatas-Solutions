...
namespace Quantum.Kata.Teleportation {
    ...
    operation SendMessage (qAlice : Qubit, qMessage : Qubit) : (Bool, Bool) {
		CNOT(qMessage,qAlice); H(qMessage);
    ...
    operation StandardTeleport (qAlice : Qubit, qBob : Qubit, qMessage : Qubit) : Unit {
		H(qAlice); CNOT(qAlice,qBob);
		CNOT(qMessage,qAlice); H(qMessage);
		if(M(qMessage)==One){Z(qBob);}
		if(M(qAlice)==One){X(qBob);}    
	}
    ...
    open Microsoft.Quantum.Convert;
	operation CharlieOp1 (qAlice : Qubit, qBob : Qubit) : Int {
		CNOT(qAlice,qBob); H(qAlice);
		let m1=M(qAlice)==One?1|0;
		let m2=M(qBob)==One?1|0;
		return 2*m1+m2;
	}
	operation CharlieOp2 (q : Qubit, mInt : Int) : Unit {
		let m=IntAsBoolArray(mInt,2);
		if(m[1]){
			Z(q);
			}
		if(m[0]){
	...
	operation EntanglementSwapping () : ((Qubit, Qubit) => Int, (Qubit, Int) => Unit) {
		//use (qAlice1,qAlice2,qBob1,qBob2)=(Qubit(),Qubit(),Qubit(),Qubit());
		//H(qAlice1);CNOT(qAlice1,qAlice2);
		//H(qBob1);CNOT(qBob1,qBob2);
		//let m=CharlieOp1(qAlice2,qBob1);
		return (CharlieOp1,CharlieOp2);
	}
    ...
    operation MeasurementFreeTeleport (qAlice : Qubit, qBob : Qubit, qMessage : Qubit) : Unit {
		CNOT(qBob,qMessage);CNOT(qMessage,qBob);CNOT(qBob,qMessage);
	}
